 ...
 */
class DirectRsyncTask extends \TYPO3\Surf\Domain\Model\Task implements \TYPO3\Surf\Domain\Service\ShellCommandServiceAwareInterface
{
    ...
    public function execute(Node $node, Application $application, Deployment $deployment, array $options = array())
    {
        ...
        $releasePath = $deployment->getBackupReleasePath($application) . '/' . $node->getName() ;
        ...
        $rsyncFlags =
            isset($options['rsyncFlags']) ?
                $options['rsyncFlags'] :
                '--recursive --times --perms -L --delete --delete-excluded' . $excludeFlags;

        $backupSourcePath = "{$username}{$hostname}:".$application->getBackupSourcePath();
        $command =
            "rsync {$quietFlag} --compress {$rshFlag} {$rsyncFlags} " .
            escapeshellarg($backupSourcePath . '.') . ' ' . escapeshellarg($deployment->getLocalWorkspacePath($application));

        $this->shell->executeOrSimulate($command, $localhost, $deployment);

        $command =
            strtr("cp -RPp " . $deployment->getLocalWorkspacePath($application) . "/. $releasePath", "\t\n", '  ');
